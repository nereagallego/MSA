...
NORI_NAMESPACE_BEGIN
...

            {
                ...
                for(Emitter *emitter:lights){
                    ...
                    while(n_emitted++ < photon_count){
                        ...
                        Color3f power = flux, throughput = flux;
                        Vector3f wi = emitterRecord.wi;
                        BSDFQueryRecord bRec(x.toLocal(-wi), x.uv);
                        ...
                        while(true){
                            ...
                            if(sampledFromEmitter){
                                ...
                                cout << "f1 " << f.r() << " "  << f.g() << " " << f.b() << endl;
                                sampledFromEmitter = false;
                            }else {
                                Color3f f = bsdf->sample(bRec, sampler->next2D());
                                cout << "f " << f.r() << " "  << f.g() << " " << f.b() << endl;
                                throughput *= f;
                                cout << "Throughput " << throughput.r() << " "  << throughput.g() << " " << throughput.b() << endl;
                            }

                            Ray3f ray(x.p, x.toWorld(bRec.wo), Epsilon, INFINITY);
                            wi = - x.toWorld(bRec.wo);
                            if (!scene->rayIntersect(ray,y)) {
                            ...
                            if(y.mesh->getBSDF()->isDiffuse()){
                                ...
                                // cout << "Throughput " << throughput.r() << " "  << throughput.g() << " " << throughput.b() << endl;
                                cout << "power " << photon.power.r() << " "  << photon.power.g() << " " << photon.power.b() << endl;
    ...
    NORI_REGISTER_CLASS(PhotonMappingIntegrator, "photon_mapping2");
NORI_NAMESPACE_END
